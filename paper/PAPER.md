...
Laporta and Remiddi (1996) obtained the analytic three-loop result. The $72$ distinct three-loop topologies combine into an expression involving polylogarithms up to $\mathrm{Li}_4(1/2)$, the Riemann zeta function at integer arguments, and powers of $\ln 2$, with numerical value $c_3 = 1.181\,234\,016\,816$. Kinoshita and Nio (2006), following two decades of automated diagram evaluation by the Kinoshita group, gave the four-loop coefficient $c_4 = -1.912\,246$, refined subsequently in Aoyama, Kinoshita and Nio (2019); the relevant amplitudes do not admit a closed-form analytic evaluation. Aoyama, Hayakawa, Kinoshita and Nio (2012) gave the five-loop coefficient $c_5 = 6.7349$, requiring the evaluation of $12{,}672$ Feynman diagrams. Volkov (2019) has since recomputed substantial portions of the five-loop result using an independent numerical technique, with results consistent at the published precision. The five-loop coefficient nonetheless rests at full precision on the single calculational effort of the Kinoshita group; the independent checks remain partial, and a complete independent five-loop evaluation is the principal unverified link in the QED prediction.
...
The hierarchy of magnitudes (QED of order $10^{-3}$, hadronic of order $10^{-12}$, weak of order $10^{-14}$) means that for the electron the QED sector dominates by nine orders of magnitude over the hadronic effect and a further hundred over the weak. The hierarchy reverses substantially for the muon (Hoecker and Marciano, 2022), where the mass-dependent hadronic and weak contributions both scale roughly as the square of the lepton mass and become comparable to the QED uncertainty.

The mass-dependent QED contribution from internal muon and tau loops, denoted $r_{\text{mass-dep}}$ in the decomposition, contributes approximately $+2.84 \times 10^{-12}$ at the present value of $\alpha$, dominated by the four-loop $A_2^{(8)}(m_e/m_\mu)$ piece. The contribution is small at the electron precision and was historically the last piece to be brought to the same accuracy as the universal coefficients.
...
The stratigraphic reading of $1.001\,159\,652\,180\,59$ is more than a mnemonic. Each digit traces to a specific calculation or apparatus, with a specific date, a specific group of authors, and a specific technique. The integer is Dirac (1928). The next several digits are Schwinger (1948). The next handful are Sommerfield, Petermann, Laporta, and Remiddi over the period 1957 to 1996. The digits at the part-per-trillion level are the Penning trap, Washington to Harvard to Northwestern, over the period 1987 to 2023. The digit at which the comparison saturates is the Berkeley-Paris discrepancy of 2018 and 2020. The composition is finite, layered, and dateable.
